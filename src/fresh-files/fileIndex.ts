...
export class FileIndex {
  ...
  private dirStatsCache: Map<string, DirStats> | null = null;
  ...
  rebuild(files: Map<AbsolutePath, FileMetadata>): void {
    ...
    for (const filePath of files.keys()) {
      const normalized = filePath as string; // AbsolutePath is forward-slash normalised
  ...
  getDirectChildren(normalizedPath: string): Set<string> | undefined {
  ...
  ensureDirStats(
    ...
    passesScope: (normalizedPath: string) => boolean,
    ...
    for (const [filePath, metadata] of files) {
      ...
      const normalizedFile = filePath as string;
